...
void xfcn()
{
	...
	f = &X::foo;		// 指向X的foo成员的指针

	X x;
	int v = f(&x, 5);	// 第一个参数是类对象的地址，这个好理解
	cout << "v = " << v << endl;
}

int main ()
{
	// pdata可以指向const (or nonconst) Screen的某个string成员
	const string Screen::*pdata;  // 未初始化
	pdata = &Screen::contents;    // 赋值
	auto pdata2 = &Screen::contents; // 等价声明
